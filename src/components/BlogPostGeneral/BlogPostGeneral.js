...
import { setBlogContent, setBlogExcerpt, setBlogPath, setBlogTitle } from "~/actions/blog.actions"
...
function BlogPostGeneral() {
    const { blogPost: { title, content, excerpt, path } } = useSelector(state => state.blogReducer)
        ...
        <Card>
            ...
            <div className='h-[600px] max-h-[600px] flex flex-col py-2'>
                ...
                <JoditEditor
                    ref={editor}
                    value={content}
                    onChange={newContent => dispatch(setBlogContent(newContent))}
                />
            </div>
            <div className="py-2">
                ...
                <textarea
                    type='text'
                    value={excerpt}
                    onChange={e => dispatch(setBlogExcerpt(e.target.value))}
