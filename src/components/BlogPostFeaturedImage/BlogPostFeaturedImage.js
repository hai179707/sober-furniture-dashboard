...
import { setBlogFeaturedImage } from "~/actions/blog.actions"
...
function BlogPostFeaturedImage() {
    const { blogPost: { featuredImage } } = useSelector(state => state.blogReducer)
            ...
            uploadTask.on(
                ...
                () => {
                    getDownloadURL(uploadTask.snapshot.ref).then((url) => {
                        dispatch(setBlogFeaturedImage(url))
                        dispatch(addImage({
                            url,
                            name: image.name,
        ...
        <Card>
            ...
            <CardBody>
                {!!featuredImage
                    ?
                    <>
                        <div className="rounded-lg border border-dashed border-secondary-bg h-80 flex items-center overflow-hidden">
                        ...
                        <div className="my-2 flex justify-between">
                            ...
                            <button onClick={() => dispatch(setBlogFeaturedImage(''))} className='text-red-600'>
                                Delete
                            </button>
                        </div>
                    </>
